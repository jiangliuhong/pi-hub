...
interface NotifyRunBody {
  ...
  finishedAt?: unknown;
  /**
   * Stable run identifier generated by the Web client at send time. Required:
   * the outbox dedupes on this key, so omitting it would risk duplicate
   * notifications for the same run.
   */
  runId?: unknown;
  /**
   * Business source of the run. Only `"web"` runs are eligible for the manual
   * completion notification — Telegram/scheduler/api runs have their own
   * delivery paths and must never reach this one.
   */
  runSource?: unknown;
}
...
export async function POST(req: Request) {
  ...
  const runSource = typeof body.runSource === "string" ? body.runSource : null;

  // Only Web-owned runs may enqueue a manual completion notification. A
  // non-web source (or an unknown one) is silently skipped — Telegram and
  // scheduler runs have their own delivery paths and must not double-notify.
  if (runSource !== "web") {
  ...
  if (!runId) {
    return NextResponse.json({ ok: false, error: "runId is required for web runs" }, { status: 400 });
  }
