...
export interface ManualRunNotifyInput {
  ...
  sessionId: string;
  /**
   * Stable run identifier (UUID) generated by the Web client at send time.
   * Used as the outbox dedupe key so SSE replays, React callback re-fires,
   * or HTTP retries for the same run cannot enqueue more than one card.
   */
  runId: string;
...
export function notifyManualRun(
  ...
  // Stable per-(session, run, target) dedupe key. Using the runId instead of
  // the completion timestamp means SSE replays, React callback re-fires,
  // prompt_error-then-prompt_done sequences, and HTTP retries for the same
  // run collapse to a single notification per target chat.
  const runKey = input.runId || String(input.finishedAt ?? Date.now());
  let enqueued = 0;
  ...
  for (const target of targets) {
    try {
      ...
      const ok = writer.enqueue({
        dedupeKey: `manual-run:${input.sessionId}:${runKey}:${target.chatId}:${target.threadId}`,
