...
export async function listSessionsIncremental(): Promise<ScannedSessionInfo[]> {
	...
	// Preserve Pi Hub's custom session directory support without bypassing the
	// upstream incremental metadata cache.
	const { getConfiguredSessionDir } = await import("./session-dir");
	const configuredDir = getConfiguredSessionDir();
	const roots = [...new Set([sessionsDir, ...(configuredDir ? [configuredDir] : [])])];
	const customFiles = configuredDir
		? await readdir(configuredDir).then((entries) => entries.filter((name) => name.endsWith(".jsonl")).map((name) => join(configuredDir, name))).catch(() => [])
		: [];
	const files = [...new Set([...(await Promise.all(roots.map(enumerateSessionFiles))).flat(), ...customFiles])];
