...
test("subprocess: doctor --json --offline emits valid JSON on the contract stream", async () => {
  const { code, stdout, stderr } = await runCli(["doctor", "--json", "--offline"]);
  // A clean CI runner can legitimately be degraded because optional local
  // configuration is absent. Healthy JSON is written to stdout; degraded or
  // blocked JSON is written to stderr, as required by the CLI contract.
  assert.ok([0, 3, 4].includes(code), `unexpected exit ${code}:\n${stderr}`);
  const output = code === 0 ? stdout : stderr;
  const unexpectedOutput = code === 0 ? stderr : stdout;
  ...
  assert.equal(
    parsed.status,
    code === 0 ? "healthy" : code === 3 ? "blocked" : "degraded",
  );
});
