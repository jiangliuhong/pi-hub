...
test("prompt terminal events echo the command's runMeta for source correlation", async () => {
  // The synthesized prompt_done/prompt_error events must carry runId + runSource
  // read from the command's runMeta, so the Web notification path can tell a
  // Web-owned run apart from a Telegram/scheduler/API run on the same session.
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  ...
  assert.match(promptSource, /const runMeta = readRunMeta\(command\)/);
  // Both the success and error paths must pass runMeta to terminalEvent so the
  // correlation fields are attached (or correctly omitted when absent).
  assert.match(promptSource, /this\.emit\(terminalEvent\("prompt_done", runMeta\)\)/);
  assert.match(promptSource, /terminalEvent\(\s*"prompt_error",\s*runMeta,/);

  // terminalEvent must stamp runId + runSource only when runMeta is present,
  // so legacy/extension runs (no runMeta) still get a terminal event without
  // inventing correlation fields.
  const terminalHelperSource = source.slice(
    source.indexOf("function terminalEvent("),
    source.indexOf("export interface RpcSessionStartOptions"),
  );
  assert.match(terminalHelperSource, /if \(runMeta\)/);
  assert.match(terminalHelperSource, /event\.runId = runMeta\.runId/);
  assert.match(terminalHelperSource, /event\.runSource = runMeta\.source/);
});
