...
for (const [hubValue, webValue, timeoutMs] of [
  [undefined, "0", 0],
  [undefined, "1800000", 1_800_000],
  [undefined, "2592000000", 600_000],
  ["0", "1000", 0],
  ["1800000", "1000", 1_800_000],
  ["invalid", "1000", 600_000],
  ["", "1000", 600_000],
]) {
  test(`Hub timeout ${hubValue} / legacy timeout ${webValue} applies to an idle session`, async (t) => {
    const previousValue = process.env.PI_WEB_IDLE_TIMEOUT_MS;
    const previousHubValue = process.env.PI_HUB_IDLE_TIMEOUT_MS;
    process.env.PI_WEB_IDLE_TIMEOUT_MS = webValue;
    if (hubValue === undefined) delete process.env.PI_HUB_IDLE_TIMEOUT_MS;
    else process.env.PI_HUB_IDLE_TIMEOUT_MS = hubValue;
    try {
      ...
      if (previousHubValue === undefined) delete process.env.PI_HUB_IDLE_TIMEOUT_MS;
      else process.env.PI_HUB_IDLE_TIMEOUT_MS = previousHubValue;
    }
  });
}
