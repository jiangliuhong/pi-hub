...
test("duplicate session IDs prefer the default root across cache reloads and deletion", async (t) => {
  ...
  const original = write("same-session", [message("m1", "user", "original")]);
  const custom = join(root, "custom-sessions");
  fs.mkdirSync(custom);
  const copy = join(custom, "copy.jsonl");
  fs.writeFileSync(copy, fs.readFileSync(original, "utf8") + line(message("m2", "assistant", "newer copy", "2026-02-01T00:00:00.000Z")));
  process.env.PI_CODING_AGENT_SESSION_DIR = custom;
  for (let attempt = 0; attempt < 3; attempt++) {
    const sessions = await listAllSessions({ force: true });
    assert.equal(sessions.length, 1);
    assert.equal(sessions[0].id, "same-session");
    assert.equal(sessions[0].path, original);
    if (attempt === 1) resetSessionScanIndexForTests();
  }
  fs.rmSync(original);
  const remaining = await listAllSessions({ force: true });
  assert.equal(remaining.length, 1);
  assert.equal(remaining[0].path, copy);
});

test("custom session directories retain default sessions and survive cache reloads", async (t) => {
  const { root, write } = fixture(t);
  const previous = process.env.PI_CODING_AGENT_SESSION_DIR;
  t.after(() => {
    if (previous === undefined) delete process.env.PI_CODING_AGENT_SESSION_DIR;
  ...
  const custom = join(root, "custom-sessions");
  fs.mkdirSync(custom);
  fs.writeFileSync(join(custom, "custom.jsonl"), line({ type: "session", version: 3, id: "custom-session", cwd: root, timestamp }));
  process.env.PI_CODING_AGENT_SESSION_DIR = custom;
  for (let attempt = 0; attempt < 2; attempt++) {
    assert.deepEqual((await listSessionsIncremental()).map((s) => s.id).sort(), ["custom-session", "default-session"]);
    resetSessionScanIndexForTests();
  }
});
